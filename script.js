        ...
        switch (algorithm.value) {
            ...
            case "bubble_sort": {
                ...
                if (result_exec.innerHTML.length > 1) {
                    ...
                    chek.style.visibility = "visible"
                    btn_download.addEventListener("click", () => {
                        download(array, "Назови_меня_пожалуйста.txt", "text/plain")
            ...
            case "merge_sort": {
                ...
                console.log(`Массив времени работы : ${time_array}`)
                result_exec.innerHTML += `\nСреднее время сортировки : \t${avg_time} мс.`
                info.innerHTML += `Ядер процессорпа использовано : ${navigator.hardwareConcurrency}` + "\n"
                result_exec2.innerHTML += `Количество операций : ${counter}`
...
function binary_search_recursion(array,start,end,element){
    ...
    if(start + 1 >= end){
        console.log("Стартовый элемент дошёл до конечного.")
    ...
    if(array[middle] === element){
        ...
        console.log(`Счётчик зафиксировал ${counter} сравнений! `)
        return middle
    }
